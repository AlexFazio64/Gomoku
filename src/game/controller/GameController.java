...
public class GameController {
	...
	public void pass(int id) {
		Platform.runLater(() -> turnLbl.setText(id == 1 ? "<(°ヮ°<)" : "(>°ヮ°)>"));
	}

	public void markSpot(int game_row, int game_column, Color col) {
		gc.setFill(col);
		gc.fillOval((game_column + 1) * GS.CELLSIZE - GS.OFFSET, (game_row + 1) * GS.CELLSIZE - GS.OFFSET, GS.PAWNSIZE,
				GS.PAWNSIZE);
	}
	...
	public void markStroke(Point2D[] stroke) {
		...
		gc.strokeLine((startY + 1) * GS.CELLSIZE, (startX + 1) * GS.CELLSIZE, (endY + 1) * GS.CELLSIZE,
				(endX + 1) * GS.CELLSIZE);
	}
